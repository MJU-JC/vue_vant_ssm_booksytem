    ...
    <div id="userInfo">
        ...
        <scroll class="content" top="1" bottom="50" ref="scroll" style="padding: 0px 5px">
            <van-form @submit="onSubmit">
                <van-field readonly name="userInfo.user_name" v-model="user_name" label="用户名：" placeholder="请输入用户名" :rules="[{ required: true, message: '请填写用户名' }]" />
    ...
    export default {
        ...
        created() {
            ...
            getUserInfo().then(res => {
                ...
                this.user_name = userInfo.user_name; //用户名
                this.password = userInfo.password;  //登录密码
                this.name = userInfo.name;  //姓名
                this.gender = userInfo.gender;  //性别
                this.birthDate = userInfo.birthDate; //出生日期
                this.picker_birthDate = new Date(this.birthDate.replace(/-/g, '/'))
                this.userPhoto = userInfo.userPhoto; //用户照片
                this.userPhotoUrl = "/" + userInfo.userPhoto;
                this.telephone = userInfo.telephone;  //联系电话
                this.email = userInfo.email;  //邮箱
